...
class AnalysisEngine:
    ...
    def get_route_points(self, max_points: int = 5000) -> list[dict]:
        """Return route points for report map rendering.

        The route is derived from all detections that contain GPS. We
        deduplicate by timestamp (ms) and downsample to keep report
        generation fast on long surveys.

        Returns: list of dicts: {ts_ms, lat, lon}
        """
        dets = self.get_flat_detections(include_no_gps=False)
        if not dets:
            return []

        # Deduplicate by timestamp to reduce redundant positions
        seen = set()
        pts = []
        for d in dets:
            ts = d.get('ts_ms')
            ...
            if lat == 0.0 and lon == 0.0:
                continue
            pts.append({'ts_ms': int(ts), 'lat': lat, 'lon': lon})
        ...
        if max_points and len(pts) > max_points:
            ...
            if pts and pts[-1]['ts_ms'] != dets[-1].get('ts_ms'):
                last = dets[-1]
                try:
                    pts.append({'ts_ms': int(last.get('ts_ms')), 'lat': float(last.get('lat', 0.0)), 'lon': float(last.get('lon', 0.0))})
                except Exception:
                    pass
        return pts
