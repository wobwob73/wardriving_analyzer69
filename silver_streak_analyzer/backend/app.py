...
# Import GNSS MQTT handler for live GNSS data
GNSS_MQTT_AVAILABLE = False
try:
    from gnss_mqtt_handler import GNSSMQTTHandler, create_gnss_mqtt_handler
    GNSS_MQTT_AVAILABLE = True
except ImportError:
    pass
...
# Log GNSS Link availability
if GNSS_LINK_AVAILABLE:
    logger.info("GNSS Link module loaded successfully")
else:
    logger.warning(f"GNSS Link module not available: {GNSS_LINK_ERROR}")
...
# GNSS MQTT handler for live GNSS data from Raspberry Pi
gnss_mqtt_handler = None


def initialize_gnss_mqtt(config: dict):
    """Initialize GNSS MQTT connection for live position data"""
    global gnss_mqtt_handler
    
    if not GNSS_MQTT_AVAILABLE:
        logger.info("GNSS MQTT handler not available")
        return False
    
    if not config.get('host'):
        logger.info("GNSS MQTT not configured")
        return False
    
    try:
        gnss_mqtt_handler = GNSSMQTTHandler(
        ...
        def on_gnss_position(device_id, data):
            """Broadcast GNSS position to WebSocket clients"""
            try:
                socketio.emit('gnss_position', {
                    'device_id': device_id,
                    'data': data
                })
            except Exception as e:
        ...
        def on_gnss_status(device_id, data):
            """Broadcast GNSS status to WebSocket clients"""
            try:
                socketio.emit('gnss_device_status', {
                    'device_id': device_id,
                    'data': data
                })
            except Exception as e:
        ...
        gnss_mqtt_handler.set_position_callback(on_gnss_position)
        gnss_mqtt_handler.set_status_callback(on_gnss_status)
        
        if gnss_mqtt_handler.connect():
            logger.info(f"GNSS MQTT connected to {config['host']}:{config.get('port', 1883)}")
            return True
        else:
            logger.warning("GNSS MQTT connection failed")
            return False
            
    except Exception as e:
        logger.warning(f"GNSS MQTT initialization failed: {e}")
        return False
...
# ============= GNSS MQTT API Endpoints =============

@app.route('/api/gnss-mqtt/connect', methods=['POST'])
def gnss_mqtt_connect():
    """Connect to GNSS MQTT broker"""
    global gnss_mqtt_handler
    ...
    if not GNSS_MQTT_AVAILABLE:
        return jsonify({'error': 'GNSS MQTT module not available'}), 500
    
    # Disconnect existing if any
    if gnss_mqtt_handler and gnss_mqtt_handler.connected:
        gnss_mqtt_handler.disconnect()
    
    if initialize_gnss_mqtt(config):
        return jsonify({
            'success': True,
            'message': f"Connected to {config['host']}:{config['port']}",
            'status': gnss_mqtt_handler.get_statistics()
        })
    else:
        return jsonify({
            'success': False,
            'error': 'Failed to connect to GNSS MQTT broker'
        }), 500


@app.route('/api/gnss-mqtt/disconnect', methods=['POST'])
def gnss_mqtt_disconnect():
    """Disconnect from GNSS MQTT broker"""
    global gnss_mqtt_handler
    
    if gnss_mqtt_handler:
        gnss_mqtt_handler.disconnect()
...
@app.route('/api/gnss-mqtt/status', methods=['GET'])
def gnss_mqtt_status():
    """Get GNSS MQTT connection status"""
    global gnss_mqtt_handler
    
    if gnss_mqtt_handler:
        stats = gnss_mqtt_handler.get_statistics()
        stats['device_status'] = gnss_mqtt_handler.get_device_status()
        stats['system_status'] = gnss_mqtt_handler.get_system_status()
        return jsonify(stats)
    
    return jsonify({
        'connected': False,
        'available': GNSS_MQTT_AVAILABLE,
        'error': 'GNSS MQTT not initialized'
    })


@app.route('/api/gnss-mqtt/positions', methods=['GET'])
def gnss_mqtt_get_positions():
    """Get recent GNSS positions from MQTT"""
    global gnss_mqtt_handler
    
    if not gnss_mqtt_handler:
        return jsonify({'error': 'GNSS MQTT not initialized'}), 400
    
    count = request.args.get('count', 100, type=int)
    positions = gnss_mqtt_handler.get_latest_positions(count)
...
@app.route('/api/gnss-mqtt/positions/clear', methods=['POST'])
def gnss_mqtt_clear_positions():
    """Clear stored GNSS positions"""
    global gnss_mqtt_handler
    
    if gnss_mqtt_handler:
        gnss_mqtt_handler.clear_positions()
        return jsonify({'success': True, 'message': 'Positions cleared'})
    
    return jsonify({'error': 'GNSS MQTT not initialized'}), 400


@app.route('/api/gnss-mqtt/import-to-session', methods=['POST'])
def gnss_mqtt_import_to_session():
    """Import MQTT positions into current GNSS session"""
    global gnss_mqtt_handler, gnss_session, gnss_metrics
    ...
    positions_data = gnss_mqtt_handler.get_latest_positions(count)
    ...
    if not gnss_session:
        gnss_session = GNSSSession(
            session_id=f"mqtt_live_{datetime.now().strftime('%Y%m%d_%H%M%S')}",
            username="mqtt_import",
            role="admin",
            is_live=True
        )
    
    # Convert and add positions
    imported = 0
    for data in positions_data:
        pos = gnss_mqtt_handler.to_gnss_position(data)
        if pos:
            gnss_session.positions.append(pos)
            
            # Add receiver if not exists
            if pos.receiver_id not in gnss_session.receivers:
                gnss_session.receivers[pos.receiver_id] = ReceiverConfig(
                    id=pos.receiver_id,
                    nickname=pos.receiver_id,
    ...
    return jsonify({
        'success': True,
        'imported': imported,
        'total_positions': len(gnss_session.positions),
        'receivers': list(gnss_session.receivers.keys()),
        'session_id': gnss_session.session_id
    })
...
def gnss_status():
    ...
    return jsonify({
        ...
        'error': GNSS_LINK_ERROR if not GNSS_LINK_AVAILABLE else None,
...
@app.route('/api/gnss/export/sqlite', methods=['POST'])
def gnss_export_sqlite():
    """Export GNSS session or MQTT buffer to SQLite database"""
    global gnss_session, gnss_mqtt_handler
    
    positions = []
    receivers = {}
    session_id = f"mqtt_{int(datetime.now().timestamp())}"
    session_start = None
    session_end = None
    
    # Try session first
    if gnss_session and gnss_session.positions:
        positions = gnss_session.positions
        receivers = gnss_session.receivers
        session_id = gnss_session.session_id
        session_start = gnss_session.start_time
        session_end = gnss_session.end_time
    # Fall back to MQTT buffer
    elif gnss_mqtt_handler and gnss_mqtt_handler.connected:
        mqtt_positions = gnss_mqtt_handler.get_latest_positions(10000)
        if mqtt_positions:
            # Convert MQTT positions to a compatible format
            from gnss_models import GNSSPosition, FixType
            for mp in mqtt_positions:
                try:
                    # Parse timestamp
                    ts = mp.get('received_at') or mp.get('timestamp')
                    ...
                    pos = GNSSPosition(
                        timestamp=timestamp,
                        receiver_id=mp.get('device_id', 'unknown'),
                        lat=mp.get('latitude', 0),
                        lon=mp.get('longitude', 0),
                        alt_msl=mp.get('altitude'),
                        fix_type=FixType(mp.get('fix_type', 1)),
                        sat_count=mp.get('num_sats'),
                        hdop=mp.get('hdop'),
                        vdop=mp.get('vdop'),
                        pdop=mp.get('pdop'),
                        speed_mps=mp.get('speed'),
                        heading_deg=mp.get('heading')
                    )
                    positions.append(pos)
                    
                    # Track receivers
                    rcvr_id = mp.get('device_id', 'unknown')
                    if rcvr_id not in receivers:
                        receivers[rcvr_id] = type('Receiver', (), {
                            'id': rcvr_id,
                            'nickname': rcvr_id,
                            'protocol': type('Protocol', (), {'value': 'NMEA'})(),
                            'chip_generation': 'unknown',
                            'is_truth_receiver': False,
                            'icon_color': '#22c55e'
                        })()
                except Exception as e:
                    logger.warning(f"Failed to convert MQTT position: {e}")
                    continue
            
            if positions:
                session_start = min(p.timestamp for p in positions)
                session_end = max(p.timestamp for p in positions)
    
    if not positions:
        return jsonify({'error': 'No GNSS data available (no session or MQTT buffer)'}), 400
    ...
    try:
        ...
        # Insert session
        cursor.execute('''
            INSERT INTO session (session_id, username, start_time, end_time, notes)
            VALUES (?, ?, ?, ?, ?)
        ''', (
            session_id,
            'mqtt_export',
            session_start.isoformat() if session_start else None,
            session_end.isoformat() if session_end else None,
            'Exported from SSA'
        ))
        
        # Insert receivers
        for rcvr_id, config in receivers.items():
            cursor.execute('''
                INSERT OR IGNORE INTO receivers (receiver_id, nickname, protocol, chip_generation, is_truth, icon_color)
                VALUES (?, ?, ?, ?, ?, ?)
            ''', (
                config.id,
                config.nickname,
                config.protocol.value if hasattr(config.protocol, 'value') else str(config.protocol),
                getattr(config, 'chip_generation', 'unknown'),
                1 if getattr(config, 'is_truth_receiver', False) else 0,
                getattr(config, 'icon_color', '#22c55e')
            ))
        
        # Insert positions
        for pos in positions:
            cursor.execute('''
                INSERT INTO positions (timestamp, receiver_id, latitude, longitude, altitude_msl, altitude_hae,
                                       fix_type, sat_count, hdop, vdop, pdop, accuracy_h_m, accuracy_v_m,
            ...
            ''', (
                ...
                pos.lon,
                pos.alt_msl,
                getattr(pos, 'alt_hae', None),
                pos.fix_type.value if hasattr(pos.fix_type, 'value') else pos.fix_type,
                pos.sat_count,
                pos.hdop,
                pos.vdop,
                pos.pdop,
                getattr(pos, 'accuracy_h_m', None),
                getattr(pos, 'accuracy_v_m', None),
                pos.speed_mps,
                pos.heading_deg
            ))
        ...
        return send_file(
            ...
            as_attachment=True,
            download_name=f'gnss_tracks_{session_id}.sqlite'
        )
        
    except Exception as e:
        logger.error(f"GNSS SQLite export error: {e}")
        return jsonify({'error': str(e)}), 500


@app.route('/api/gnss/export/geojson', methods=['GET'])
def gnss_export_geojson():
    """Export GNSS session or MQTT buffer to GeoJSON"""
    global gnss_session, gnss_mqtt_handler
    ...
    try:
        # Group positions by receiver
        tracks_by_receiver = {}
        for pos in positions:
            if pos.receiver_id not in tracks_by_receiver:
                tracks_by_receiver[pos.receiver_id] = []
            tracks_by_receiver[pos.receiver_id].append(pos)
        
        features = []
        
        # Create LineString for each receiver track
        for rcvr_id, rcvr_positions in tracks_by_receiver.items():
            if len(rcvr_positions) < 2:
                continue
            
            sorted_positions = sorted(rcvr_positions, key=lambda p: p.timestamp)
            coordinates = [[pos.lon, pos.lat, pos.alt_msl or 0] for pos in sorted_positions]
            
            config = receivers.get(rcvr_id)
            ...
            feature = {
                ...
                'properties': {
                    'receiver_id': rcvr_id,
                    'nickname': config.nickname if config else rcvr_id,
                    'is_truth': getattr(config, 'is_truth_receiver', False) if config else False,
                    'point_count': len(rcvr_positions),
                    'start_time': sorted_positions[0].timestamp.isoformat(),
                    'end_time': sorted_positions[-1].timestamp.isoformat()
                }
            }
        ...
        for pos in positions:
            feature = {
                ...
                'geometry': {
                    'type': 'Point',
                    'coordinates': [pos.lon, pos.lat, pos.alt_msl or 0]
                },
                'properties': {
                    'receiver_id': pos.receiver_id,
                    'timestamp': pos.timestamp.isoformat() if pos.timestamp else None,
                    'fix_type': pos.fix_type.value if hasattr(pos.fix_type, 'value') else pos.fix_type,
                    'sat_count': pos.sat_count,
                    'hdop': pos.hdop,
                    'accuracy_h_m': getattr(pos, 'accuracy_h_m', None)
                }
            }
            features.append(feature)
        ...
        return send_file(
            ...
            as_attachment=True,
            download_name=f'gnss_tracks_{session_id}.geojson'
        )
        
    except Exception as e:
        logger.error(f"GNSS GeoJSON export error: {e}")
        return jsonify({'error': str(e)}), 500
...
<head>
    ...
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .ssa-notification.show {
            transform: translateX(0);
        }
        
        .ssa-notification-icon {
            font-size: 16px;
            font-weight: bold;
        }
        
        .ssa-notification-message {
            font-size: 13px;
            color: var(--text-primary);
        }
        
        .ssa-notification-success {
            border-color: var(--accent-green);
        }
        .ssa-notification-success .ssa-notification-icon {
            color: var(--accent-green);
        }
        
        .ssa-notification-error {
            border-color: var(--accent-red);
        }
        .ssa-notification-error .ssa-notification-icon {
            color: var(--accent-red);
        }
        
        .ssa-notification-info {
            border-color: var(--accent-blue);
        }
        .ssa-notification-info .ssa-notification-icon {
            color: var(--accent-blue);
        }
        
        .ssa-notification-warning {
            border-color: var(--accent-orange);
        }
        .ssa-notification-warning .ssa-notification-icon {
        ...
        .table-toolbar .tb-checkbox-group {
            display: flex;
            align-items: center;
        }
        
        .table-toolbar .tb-checkbox-group label {
            display: flex;
            align-items: center;
            gap: 6px;
            cursor: pointer;
        ...
        .table-toolbar .tb-checkbox-group input[type="checkbox"] {
            width: 16px;
            height: 16px;
            cursor: pointer;
        }
        ...
        .gnss-source-btn:hover {
            border-color: var(--accent-blue);
        }
        
        .gnss-source-btn.active {
        ...
        .gnss-mqtt-status-dot.connected {
            background: var(--accent-green);
        }
        ...
        .gnss-mqtt-input-group {
            display: flex;
            flex-direction: column;
            gap: 4px;
        }
        
        .gnss-mqtt-input-group label {
            font-size: 11px;
            color: var(--text-secondary);
        }
        ...
        .gnss-mqtt-btn.connect {
            background: var(--accent-green);
            color: white;
        }
        
        .gnss-mqtt-btn.disconnect {
            background: var(--accent-red);
            color: white;
        }
        ...
        .gnss-mqtt-stat-value {
            font-size: 16px;
            font-weight: 600;
            color: var(--accent-blue);
        }
        
        .gnss-mqtt-stat-label {
            font-size: 10px;
            color: var(--text-secondary);
        }
...
<body>
    <div class="app-container">
        ...
        <header class="header">
            <div class="header-left">
                ...
                <span class="header-badge">SSA v4.8.0</span>
            </div>
        ...
        <div class="main-content">
            ...
            <div class="table-container" id="table-section" style="display: none;">
                <div class="table-toolbar">
                    ...
                    <div class="tb-group tb-checkbox-group">
                        <label>
                            <input type="checkbox" id="tb-dup-ssid" onchange="setTableFilter('dupSSID', this.checked)">
                            Duplicate SSIDs Only
                        </label>
                    </div>
            ...
            <div class="gnss-container" id="gnss-section" style="display: none;">
                ...
                <div class="gnss-content">
                    <!-- Source Toggle -->
                    <div class="gnss-source-toggle">
                        <button class="gnss-source-btn active" id="gnss-src-csv" onclick="setGNSSSource('csv')">📤 CSV Upload</button>
                        <button class="gnss-source-btn" id="gnss-src-mqtt" onclick="setGNSSSource('mqtt')">📡 Live MQTT</button>
                    </div>
                    
                    <!-- Upload Section (CSV mode) -->
                    <div class="gnss-upload-section" id="gnss-csv-section">
                    ...
                    <!-- MQTT Section (MQTT mode) -->
                    <div class="gnss-mqtt-section" id="gnss-mqtt-section" style="display: none;">
                        <div class="gnss-mqtt-header">
                            <span class="gnss-mqtt-title">🔌 MQTT Connection</span>
                            <div class="gnss-mqtt-status">
                                <span class="gnss-mqtt-status-dot" id="gnss-mqtt-status-dot"></span>
                                <span id="gnss-mqtt-status-text">Disconnected</span>
                            </div>
                        </div>
                        
                        <div class="gnss-mqtt-form">
                            <div class="gnss-mqtt-input-group">
                                <label>Broker Host (Pi IP)</label>
                                <input type="text" class="gnss-mqtt-input" id="gnss-mqtt-host" placeholder="10.42.0.223">
                            </div>
                            <div class="gnss-mqtt-input-group">
                                <label>Port</label>
                                <input type="number" class="gnss-mqtt-input" id="gnss-mqtt-port" value="1883">
                            </div>
                            <button class="gnss-mqtt-btn connect" id="gnss-mqtt-connect-btn" onclick="toggleGNSSMqtt()">Connect</button>
                        </div>
                        
                        <div class="gnss-mqtt-stats" id="gnss-mqtt-stats" style="display: none;">
                            <div class="gnss-mqtt-stat">
                                <div class="gnss-mqtt-stat-value" id="gnss-mqtt-positions">0</div>
                                <div class="gnss-mqtt-stat-label">Positions</div>
                            </div>
                            <div class="gnss-mqtt-stat">
                                <div class="gnss-mqtt-stat-value" id="gnss-mqtt-devices">0</div>
                                <div class="gnss-mqtt-stat-label">Devices</div>
                            </div>
                            <div class="gnss-mqtt-stat">
                                <div class="gnss-mqtt-stat-value" id="gnss-mqtt-rate">0</div>
                                <div class="gnss-mqtt-stat-label">Hz</div>
                            </div>
                            <div class="gnss-mqtt-stat">
                                <div class="gnss-mqtt-stat-value" id="gnss-mqtt-sats">-</div>
                                <div class="gnss-mqtt-stat-label">Sats</div>
                            </div>
                        </div>
                        
                        <div class="gnss-mqtt-actions" id="gnss-mqtt-actions" style="display: none;">
                            <button class="gnss-mqtt-action-btn" onclick="importGNSSMqttPositions()">📥 Import to Session</button>
                            <button class="gnss-mqtt-action-btn" onclick="clearGNSSMqttPositions()">🗑️ Clear Buffer</button>
                        </div>
                    </div>
                    
                    <!-- Mini Map -->
                    <div class="gnss-map-section" id="gnss-map-section" style="display: none;">
                        <h3>🗺️ Receiver Positions</h3>
                        <div class="gnss-mini-map" id="gnss-mini-map"></div>
                    </div>
            ...
            <div class="side-panel right-panel" id="right-panel">
                ...
                <!-- Stats - SOI Mode -->
                <div class="sidebar-section" id="soi-stats-section">
                ...
                <!-- Stats - GNSS Mode (hidden by default) -->
                <div class="sidebar-section" id="gnss-stats-section" style="display: none;">
                    <div class="section-title">📡 GNSS Statistics</div>
                    <div class="stats-grid">
                        <div class="stat-card">
                            <div class="stat-value" id="gnss-stat-receivers">0</div>
                            <div class="stat-label">Receivers</div>
                        </div>
                        <div class="stat-card">
                            <div class="stat-value" id="gnss-stat-positions">0</div>
                            <div class="stat-label">Positions</div>
                        </div>
                        <div class="stat-card small">
                            <div class="stat-value" id="gnss-stat-avg-sats" style="color: var(--accent-green);">-</div>
                            <div class="stat-label">Avg SVs</div>
                        </div>
                        <div class="stat-card small">
                            <div class="stat-value" id="gnss-stat-constellations" style="color: var(--accent-blue);">-</div>
                            <div class="stat-label">Constell.</div>
                        </div>
                    </div>
                    
                    <!-- GNSS Quality Stats -->
                    <div class="filter-stats-section" style="margin-top: 12px; padding-top: 12px; border-top: 1px solid var(--border-color);">
                        <div class="filter-label" style="margin-bottom: 8px;">📊 Quality Metrics</div>
                        <div class="stats-grid">
                            <div class="stat-card small">
                                <div class="stat-value" id="gnss-stat-hdop" style="color: var(--accent-green);">-</div>
                                <div class="stat-label">Avg HDOP</div>
                            </div>
                            <div class="stat-card small">
                                <div class="stat-value" id="gnss-stat-fix-rate" style="color: var(--accent-blue);">-</div>
                                <div class="stat-label">Fix Rate</div>
                            </div>
                            <div class="stat-card small">
                                <div class="stat-value" id="gnss-stat-cep50" style="color: var(--accent-orange);">-</div>
                                <div class="stat-label">CEP50 (m)</div>
                            </div>
                            <div class="stat-card small">
                                <div class="stat-value" id="gnss-stat-2drms" style="color: var(--accent-red);">-</div>
                                <div class="stat-label">2DRMS (m)</div>
                            </div>
                        </div>
                    </div>
                    ...
                    <!-- Lock View Controls -->
                    <div class="filter-stats-section" style="margin-top: 12px; padding-top: 12px; border-top: 1px solid var(--border-color);">
                        <div class="filter-label" style="margin-bottom: 8px;">🔒 Lock View</div>
                        <div style="display: flex; flex-direction: column; gap: 8px;">
                            <select id="gnss-lock-receiver" style="padding: 6px 10px; border-radius: 6px; background: var(--bg-tertiary); border: 1px solid var(--border-color); color: var(--text-primary); font-size: 12px;">
                                <option value="">Select receiver...</option>
                            </select>
                            <div style="display: flex; gap: 8px;">
                                <button id="gnss-lock-btn" onclick="toggleGNSSLockView()" style="flex: 1; padding: 6px 12px; border-radius: 6px; border: 1px solid var(--border-color); background: var(--bg-tertiary); color: var(--text-primary); cursor: pointer; font-size: 12px;">
                                    🔓 Unlocked
                                </button>
                                <button onclick="centerOnLockedReceiver()" style="padding: 6px 12px; border-radius: 6px; border: 1px solid var(--border-color); background: var(--bg-tertiary); color: var(--text-primary); cursor: pointer; font-size: 12px;" title="Center on receiver">
                                    🎯
                                </button>
                            </div>
                            <div style="font-size: 10px; color: var(--text-secondary);">
                                Lock keeps map centered on receiver at current zoom
                            </div>
                        </div>
                    </div>
                    
                    <!-- GNSS MQTT Status -->
                    <div class="filter-stats-section" id="gnss-mqtt-mini-status" style="margin-top: 12px; padding-top: 12px; border-top: 1px solid var(--border-color);">
                        <div class="filter-label" style="margin-bottom: 8px;">📡 Live MQTT</div>
                        <div style="display: flex; align-items: center; gap: 8px;">
                            <span class="gnss-mqtt-status-dot" id="gnss-mqtt-mini-dot"></span>
                            <span id="gnss-mqtt-mini-text" style="font-size: 12px;">Disconnected</span>
                            <span style="margin-left: auto; font-size: 12px; color: var(--accent-blue);" id="gnss-mqtt-mini-count">0 pos</span>
                        </div>
                    </div>
                </div>
    ...
    <div class="modal-overlay" id="settings-modal" onclick="closeSettingsFromBackdrop(event)">
        <div class="modal" onclick="event.stopPropagation()">
            ...
            <div class="settings-tabs">
                ...
                <div class="settings-tab" id="settings-tab-gnss" onclick="showSettingsPane('gnss')">GNSS Link</div>
            ...
            <div class="settings-pane" id="settings-pane-gnss">
            <div style="margin-top: 14px;">
                <div class="filter-label">📡 GNSS Receiver Colors</div>
                <div style="font-size: 11px; color: var(--text-secondary); margin-bottom: 8px;">Configure display colors for each receiver. Upload JSON or edit directly.</div>
                
                <div id="gnss-receiver-colors-list" style="max-height: 180px; overflow: auto; border: 1px solid var(--border-color); border-radius: 10px; padding: 10px; margin-bottom: 10px;">
                    <div style="color: var(--text-secondary); font-size: 12px;">No receivers configured. Colors will be auto-assigned.</div>
                </div>
                
                <div style="display: flex; gap: 10px; margin-bottom: 16px;">
                    <button class="action-btn" onclick="triggerGNSSColorUpload()">📤 Upload Colors JSON</button>
                    <button class="action-btn" onclick="exportGNSSColors()">📥 Export Colors</button>
                    <input type="file" id="gnss-color-upload" accept="application/json" style="display:none" onchange="uploadGNSSColorsJson(this.files[0])">
                </div>
                
                <div class="filter-label">📍 Points of Interest (POI)</div>
                <div style="font-size: 11px; color: var(--text-secondary); margin-bottom: 8px;">Add reference points with configurable radius circles.</div>
                
                <div id="poi-list" style="max-height: 200px; overflow: auto; border: 1px solid var(--border-color); border-radius: 10px; padding: 10px; margin-bottom: 10px;">
                    <div style="color: var(--text-secondary); font-size: 12px;">No POIs defined. Add one below.</div>
                </div>
                
                <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 8px; margin-bottom: 10px;">
                    <div>
                        <label style="font-size: 11px; color: var(--text-secondary);">Label</label>
                        <input type="text" id="poi-label" placeholder="Reference Point" style="width: 100%; padding: 6px 10px; border-radius: 6px; background: var(--surface-light); border: 1px solid var(--border-color); color: var(--text-primary);">
                    </div>
                    <div>
                        <label style="font-size: 11px; color: var(--text-secondary);">Color</label>
                        <input type="color" id="poi-color" value="#ff6b6b" style="width: 100%; height: 32px; padding: 2px; border-radius: 6px; background: var(--surface-light); border: 1px solid var(--border-color);">
                    </div>
                </div>
                <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 8px; margin-bottom: 10px;">
                    <div>
                        <label style="font-size: 11px; color: var(--text-secondary);">Latitude</label>
                        <input type="number" id="poi-lat" step="0.000001" placeholder="35.254123" style="width: 100%; padding: 6px 10px; border-radius: 6px; background: var(--surface-light); border: 1px solid var(--border-color); color: var(--text-primary);">
                    </div>
                    <div>
                        <label style="font-size: 11px; color: var(--text-secondary);">Longitude</label>
                        <input type="number" id="poi-lon" step="0.000001" placeholder="-78.958769" style="width: 100%; padding: 6px 10px; border-radius: 6px; background: var(--surface-light); border: 1px solid var(--border-color); color: var(--text-primary);">
                    </div>
                </div>
                <div style="display: grid; grid-template-columns: 1fr 1fr 1fr; gap: 8px; margin-bottom: 10px;">
                    <div>
                        <label style="font-size: 11px; color: var(--text-secondary);">Radius 1 (m)</label>
                        <input type="number" id="poi-radius1" value="100" min="0" style="width: 100%; padding: 6px 10px; border-radius: 6px; background: var(--surface-light); border: 1px solid var(--border-color); color: var(--text-primary);">
                    </div>
                    <div>
                        <label style="font-size: 11px; color: var(--text-secondary);">Radius 2 (m)</label>
                        <input type="number" id="poi-radius2" value="500" min="0" style="width: 100%; padding: 6px 10px; border-radius: 6px; background: var(--surface-light); border: 1px solid var(--border-color); color: var(--text-primary);">
                    </div>
                    <div>
                        <label style="font-size: 11px; color: var(--text-secondary);">Radius 3 (m)</label>
                        <input type="number" id="poi-radius3" value="1000" min="0" style="width: 100%; padding: 6px 10px; border-radius: 6px; background: var(--surface-light); border: 1px solid var(--border-color); color: var(--text-primary);">
                    </div>
                </div>
                <div style="display: flex; gap: 10px;">
                    <button class="action-btn" onclick="addPOI()">➕ Add POI</button>
                    <button class="action-btn" onclick="clearAllPOIs()">🗑️ Clear All</button>
                </div>
                
                <div style="margin-top: 16px;">
                    <div class="filter-label">💾 Export GNSS Tracks</div>
                    <div style="font-size: 11px; color: var(--text-secondary); margin-bottom: 8px;">Export recorded GNSS tracks to database file.</div>
                    <div style="display: flex; gap: 10px;">
                        <button class="action-btn" onclick="exportGNSSToSQLite()">📦 Export SQLite</button>
                        <button class="action-btn" onclick="exportGNSSToGeoJSON()">📍 Export GeoJSON</button>
                    </div>
                </div>
            </div>
            </div>
    ...
    <script>
        ...
        function showNotification(message, type = 'info') {
            ...
            const existing = document.getElementById('ssa-notification');
            if (existing) existing.remove();
            
            const notification = document.createElement('div');
            notification.id = 'ssa-notification';
            notification.className = `ssa-notification ssa-notification-${type}`;
            ...
            notification.innerHTML = `
                <span class="ssa-notification-icon">${icons[type] || icons.info}</span>
                <span class="ssa-notification-message">${message}</span>
            `;
...
async function saveReportOptions() {
        ...
        function showSettingsPane(name) {
            const panes = ['general','report','charts','gnss','whitelist','about'];
...
// ------------------- Layer helpers -------------------
        ...
        function getVisibleTableRows() {
            ...
            const dupSSIDOnly = tableFilters.dupSSID === true;
            
            // Build duplicate SSID set if filter is active
            let duplicateSSIDs = new Set();
            if (dupSSIDOnly) {
                const ssidCounts = {};
                accessPoints.forEach(ap => {
                    const ssid = (ap.name || '').trim().toLowerCase();
                    if (ssid && ssid !== '(hidden)') {
                        ssidCounts[ssid] = (ssidCounts[ssid] || 0) + 1;
                    }
                });
                Object.entries(ssidCounts).forEach(([ssid, count]) => {
                    if (count > 1) duplicateSSIDs.add(ssid);
                });
            }
            ...
            accessPoints.forEach(ap => {
                ...
                // Duplicate SSID filter
                if (dupSSIDOnly && !duplicateSSIDs.has(name)) return;
...
// Update table view
        function updateTable() {
            ...
            // Move selected AP to top of list if one is selected
            if (selectedAP) {
                const selectedKey = String(selectedAP.id).toLowerCase();
                const selectedIndex = rows.findIndex(ap => String(ap.id).toLowerCase() === selectedKey);
                if (selectedIndex > 0) {
        ...
        function setView(view) {
            ...
            // Stats panels are now managed by updateGNSSReceiverLayer based on connection status
            // Only force GNSS stats when in GNSS view
            if (view === 'gnss') {
                document.getElementById('soi-stats-section').style.display = 'none';
                document.getElementById('gnss-stats-section').style.display = 'block';
            }
            
            if (view === 'map') {
                setTimeout(() => map.invalidateSize(), 100);
                // Show GNSS receivers on main map if session active
                updateGNSSReceiverLayer();
            }
            if (view === 'gnss') {
                loadGNSSStatus();
            }
        ...
        async function loadGNSSStatus() {
            try {
                console.log('GNSS Status: Checking module availability');
                const response = await fetch('/api/gnss/status');
                const data = await response.json();
                console.log('GNSS Status:', data);
                
                if (!data.available) {
                    showNotification('GNSS Link module not available - check server logs', 'warning');
                    return;
                }
        ...
        async function handleGNSSFileUpload(event) {
            ...
            console.log('GNSS Upload: Starting upload of', files.length, 'files');
            
            const formData = new FormData();
            for (const file of files) {
                formData.append('files', file);
                console.log('GNSS Upload: Added file', file.name);
            }
            ...
            try {
                ...
                console.log('GNSS Upload: Response status', response.status);
                const data = await response.json();
                console.log('GNSS Upload: Response data', data);
                ...
                if (data.success) {
                    ...
                    console.error('GNSS Upload: Error', data.error);
                    showNotification('Failed to load GNSS data: ' + (data.error || 'Unknown error'), 'error');
                }
            } catch (error) {
                console.error('GNSS Upload: Exception', error);
        ...
        function updateGNSSUI(metrics, receivers) {
            ...
            // Show and update mini map with session positions
            document.getElementById('gnss-map-section').style.display = 'block';
            updateGNSSSessionMap(receivers);
        }
        
        async function updateGNSSSessionMap(receivers) {
            // Initialize map if not exists
            if (!gnssMiniMap) {
                initGNSSMiniMap();
            }
            
            if (!gnssMiniMap) return;
            
            // Clear existing markers
            Object.values(gnssMiniMapMarkers).forEach(m => m.remove());
            gnssMiniMapMarkers = {};
            if (gnssTrackLine) {
                gnssTrackLine.remove();
                gnssTrackLine = null;
            }
            
            try {
                // Fetch positions from session
                const response = await fetch('/api/gnss/positions?limit=500');
                const data = await response.json();
                
                if (!data.positions || data.positions.length === 0) return;
                
                const positions = data.positions;
                const tracksByReceiver = {};
                const latestByReceiver = {};
                const receiverColors = {};
                
                // Get receiver colors
                for (const rcvr of receivers) {
                    receiverColors[rcvr.id] = rcvr.icon_color || '#22c55e';
                }
                
                // Group positions by receiver
                for (const pos of positions) {
                    const rcvrId = pos.receiver_id;
                    if (!tracksByReceiver[rcvrId]) {
                        tracksByReceiver[rcvrId] = [];
                    }
                    if (pos.lat && pos.lon && Math.abs(pos.lat) <= 90 && Math.abs(pos.lon) <= 180) {
                        tracksByReceiver[rcvrId].push([pos.lat, pos.lon]);
                        latestByReceiver[rcvrId] = pos;
                    }
                }
                
                // Draw tracks and markers for each receiver
                let bounds = [];
                for (const [rcvrId, track] of Object.entries(tracksByReceiver)) {
                    const color = receiverColors[rcvrId] || '#22c55e';
                    ...
                    const latest = latestByReceiver[rcvrId];
                    if (latest) {
                        ...
                        const icon = L.divIcon({
                            ...
                            html: `<div style="background: ${color}; width: 12px; height: 12px; border-radius: 50%; border: 2px solid white; box-shadow: 0 0 6px rgba(0,0,0,0.5);"></div>`,
                            iconSize: [16, 16],
                            iconAnchor: [8, 8]
                        });
                        
                        gnssMiniMapMarkers[rcvrId] = L.marker(latlng, { icon })
                            .addTo(gnssMiniMap)
                            .bindPopup(`<b>${rcvrId}</b><br>Sats: ${latest.sat_count || '-'}<br>HDOP: ${latest.hdop?.toFixed(2) || '-'}`);
                    }
                }
                ...
                if (bounds.length > 0) {
                    if (bounds.length === 1) {
                        gnssMiniMap.setView(bounds[0], 16);
                    } else {
                        gnssMiniMap.fitBounds(bounds, { padding: [20, 20] });
                    }
                }
                
            } catch (error) {
                console.error('Failed to update GNSS session map:', error);
            }
        }
        ...
        async function clearGNSSSession() {
            ...
            try {
                ...
                document.getElementById('gnss-map-section').style.display = 'none';
        ...
        let gnssMqttConnected = false;
        let gnssMqttStatusInterval = null;
        let gnssMiniMap = null;
        let gnssMiniMapMarkers = {};
        let gnssTrackLine = null;
        
        function setGNSSSource(source) {
            document.getElementById('gnss-src-csv').classList.toggle('active', source === 'csv');
            document.getElementById('gnss-src-mqtt').classList.toggle('active', source === 'mqtt');
            document.getElementById('gnss-csv-section').style.display = source === 'csv' ? 'block' : 'none';
            document.getElementById('gnss-mqtt-section').style.display = source === 'mqtt' ? 'block' : 'none';
        ...
        async function toggleGNSSMqtt() {
            if (gnssMqttConnected) {
                await disconnectGNSSMqtt();
            } else {
                await connectGNSSMqtt();
            }
        }
        
        async function connectGNSSMqtt() {
            const host = document.getElementById('gnss-mqtt-host').value.trim();
            const port = parseInt(document.getElementById('gnss-mqtt-port').value) || 1883;
            
            if (!host) {
                showNotification('Please enter the Pi IP address', 'warning');
                return;
            }
            
            try {
                showNotification('Connecting to GNSS MQTT...', 'info');
                
                const response = await fetch('/api/gnss-mqtt/connect', {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' },
                ...
                if (data.success) {
                    gnssMqttConnected = true;
                    updateGNSSMqttUI(true);
                    showNotification(`Connected to ${host}:${port}`, 'success');
                    
                    // Start status polling
                    gnssMqttStatusInterval = setInterval(loadGNSSMqttStatus, 1000);
        ...
        async function disconnectGNSSMqtt() {
            try {
                await fetch('/api/gnss-mqtt/disconnect', { method: 'POST' });
                
                gnssMqttConnected = false;
                updateGNSSMqttUI(false);
                
                if (gnssMqttStatusInterval) {
                    clearInterval(gnssMqttStatusInterval);
                    gnssMqttStatusInterval = null;
                }
                
                showNotification('Disconnected from GNSS MQTT', 'info');
            } catch (error) {
                showNotification('Disconnect error: ' + error.message, 'error');
            }
        }
        
        function updateGNSSMqttUI(connected) {
            const dot = document.getElementById('gnss-mqtt-status-dot');
            const text = document.getElementById('gnss-mqtt-status-text');
            const btn = document.getElementById('gnss-mqtt-connect-btn');
            const stats = document.getElementById('gnss-mqtt-stats');
            const actions = document.getElementById('gnss-mqtt-actions');
            const mapSection = document.getElementById('gnss-map-section');
            
            dot.classList.toggle('connected', connected);
            text.textContent = connected ? 'Connected' : 'Disconnected';
            btn.textContent = connected ? 'Disconnect' : 'Connect';
            btn.classList.toggle('connect', !connected);
            ...
            mapSection.style.display = connected ? 'block' : 'none';
        }
        
        async function loadGNSSMqttStatus() {
            try {
                const response = await fetch('/api/gnss-mqtt/status');
                const data = await response.json();
                
                if (data.connected) {
                    document.getElementById('gnss-mqtt-positions').textContent = data.positions_received || 0;
                    document.getElementById('gnss-mqtt-devices').textContent = data.devices_tracked || 0;
                    document.getElementById('gnss-mqtt-rate').textContent = '1';  // Assume 1Hz
                    
                    // Get satellite count from first device
                    const devices = Object.values(data.device_status || {});
                    if (devices.length > 0) {
                        document.getElementById('gnss-mqtt-sats').textContent = devices[0].num_sats || '-';
                    }
                    
                    // Update map with latest positions
                    await updateGNSSMiniMapPositions();
                } else if (gnssMqttConnected) {
                    // Connection lost
                    gnssMqttConnected = false;
                    updateGNSSMqttUI(false);
                    if (gnssMqttStatusInterval) {
                ...
                console.error('Failed to load GNSS MQTT status:', error);
            }
        }
        
        async function importGNSSMqttPositions() {
            try {
                showNotification('Importing positions to session...', 'info');
                
                const response = await fetch('/api/gnss-mqtt/import-to-session?count=1000', {
                    method: 'POST'
                });
                ...
                if (data.success) {
                    gnssSession = data;
                    await loadGNSSData();
                    showNotification(`Imported ${data.imported} positions from ${data.receivers.length} receiver(s)`, 'success');
                } else {
        ...
        async function clearGNSSMqttPositions() {
            try {
                await fetch('/api/gnss-mqtt/positions/clear', { method: 'POST' });
                document.getElementById('gnss-mqtt-positions').textContent = '0';
                ...
                showNotification('Position buffer cleared', 'success');
            } catch (error) {
                showNotification('Clear failed: ' + error.message, 'error');
            }
        }
        
        function initGNSSMiniMap() {
            if (gnssMiniMap) {
                gnssMiniMap.remove();
            }
            
            const container = document.getElementById('gnss-mini-map');
            if (!container) return;
            
            gnssMiniMap = L.map('gnss-mini-map', {
                zoomControl: true,
                attributionControl: false
            }).setView([35.25, -78.96], 15);
        ...
        async function updateGNSSMiniMapPositions() {
            if (!gnssMiniMap) return;
            
            try {
                const response = await fetch('/api/gnss-mqtt/positions?count=100');
                const data = await response.json();
                ...
                // Group by device and collect track points
                for (const pos of positions) {
                    if (pos.latitude && pos.longitude && 
                        Math.abs(pos.latitude) <= 90 && Math.abs(pos.longitude) <= 180) {
                        latestByDevice[pos.device_id] = pos;
                        trackPoints.push([pos.latitude, pos.longitude]);
                    }
                }
                
                // Update device markers
                for (const [deviceId, pos] of Object.entries(latestByDevice)) {
                    const latlng = [pos.latitude, pos.longitude];
                    
                    if (gnssMiniMapMarkers[deviceId]) {
                        gnssMiniMapMarkers[deviceId].setLatLng(latlng);
                    } else {
                        const icon = L.divIcon({
                            className: 'gnss-marker',
                            html: `<div style="background: #22c55e; width: 12px; height: 12px; border-radius: 50%; border: 2px solid white; box-shadow: 0 0 6px rgba(0,0,0,0.5);"></div>`,
                            iconSize: [16, 16],
                            iconAnchor: [8, 8]
                        });
                        
                        gnssMiniMapMarkers[deviceId] = L.marker(latlng, { icon })
                            .addTo(gnssMiniMap)
                            .bindPopup(`<b>${deviceId}</b><br>Sats: ${pos.num_sats || '-'}<br>HDOP: ${pos.hdop?.toFixed(2) || '-'}`);
                    }
                }
                ...
                if (trackPoints.length > 1) {
                    if (gnssTrackLine) {
                        gnssTrackLine.setLatLngs(trackPoints);
                    } else {
                        gnssTrackLine = L.polyline(trackPoints, {
                            color: '#3b82f6',
                            weight: 2,
                            opacity: 0.7
                        }).addTo(gnssMiniMap);
                ...
                // Pan to latest position (only on first update)
                const latestPos = positions[positions.length - 1];
                if (latestPos && Object.keys(gnssMiniMapMarkers).length === 1) {
                    gnssMiniMap.setView([latestPos.latitude, latestPos.longitude], 16);
                }
                
            } catch (error) {
                console.error('Failed to update GNSS mini map:', error);
            }
        }
        
        // ============= GNSS Receiver Layer on Main Map =============
        
        let gnssReceiverLayer = null;
        let gnssTrackLayers = {};
        let gnssReceiverMarkers = {};
        let gnssReceiverColors = {};  // User-configured colors
        const defaultReceiverColors = ['#22c55e', '#3b82f6', '#f59e0b', '#ef4444', '#8b5cf6', '#06b6d4', '#ec4899', '#84cc16'];
        
        // Lock view state
        let gnssLockView = false;
        let gnssLockedReceiver = null;
        let gnssLockedZoom = null;
        
        function getReceiverColor(receiverId) {
            if (gnssReceiverColors[receiverId]) {
                return gnssReceiverColors[receiverId];
            }
            // Auto-assign from defaults
            const receivers = Object.keys(gnssReceiverMarkers);
            const idx = receivers.indexOf(receiverId);
            return defaultReceiverColors[idx % defaultReceiverColors.length];
        }
        
        function toggleGNSSLockView() {
            const select = document.getElementById('gnss-lock-receiver');
            const btn = document.getElementById('gnss-lock-btn');
            
            if (gnssLockView) {
                // Unlock
                gnssLockView = false;
                gnssLockedReceiver = null;
                gnssLockedZoom = null;
                ...
                // Lock
                const selectedReceiver = select.value;
                if (!selectedReceiver) {
                    showNotification('Select a receiver to lock on', 'warning');
                    return;
                }
                gnssLockView = true;
                gnssLockedReceiver = selectedReceiver;
                gnssLockedZoom = map.getZoom();
                btn.innerHTML = '🔒 Locked: ' + selectedReceiver;
                btn.style.background = 'rgba(34, 197, 94, 0.2)';
                btn.style.borderColor = 'var(--accent-green)';
                
                // Center immediately
                centerOnLockedReceiver();
            }
        }
        
        function centerOnLockedReceiver() {
            if (!gnssLockedReceiver || !gnssReceiverMarkers[gnssLockedReceiver]) {
                return;
            }
            const latlng = gnssReceiverMarkers[gnssLockedReceiver].getLatLng();
            if (gnssLockedZoom) {
                map.setView(latlng, gnssLockedZoom, { animate: true, duration: 0.3 });
            } else {
                map.panTo(latlng, { animate: true, duration: 0.3 });
            }
        }
        
        async function updateGNSSReceiverLayer() {
            if (!map) return;
            
            try {
                // Check if we have MQTT connection or session data
                const mqttStatus = await fetch('/api/gnss-mqtt/status').then(r => r.json()).catch(() => null);
                const gnssStatus = await fetch('/api/gnss/status').then(r => r.json()).catch(() => null);
                
                let positions = [];
                let isConnected = false;
                
                // Try MQTT positions first
                if (mqttStatus && mqttStatus.connected && mqttStatus.positions_stored > 0) {
                    const mqttData = await fetch('/api/gnss-mqtt/positions?count=200').then(r => r.json());
                    positions = mqttData.positions || [];
                    isConnected = true;
                    
                    // Update MQTT mini status
                    document.getElementById('gnss-mqtt-mini-dot').classList.add('connected');
                    document.getElementById('gnss-mqtt-mini-text').textContent = 'Connected';
                    document.getElementById('gnss-mqtt-mini-count').textContent = mqttStatus.positions_stored + ' pos';
                }
                // Fall back to session positions
                else if (gnssStatus && gnssStatus.session_active) {
                    const sessionData = await fetch('/api/gnss/positions?limit=200').then(r => r.json());
                    positions = (sessionData.positions || []).map(p => ({
                        device_id: p.receiver_id,
                        latitude: p.lat,
                        longitude: p.lon,
                        num_sats: p.sat_count,
                        hdop: p.hdop
                    }));
                    isConnected = true;
                } else {
                    // Update MQTT mini status - disconnected
                    document.getElementById('gnss-mqtt-mini-dot').classList.remove('connected');
                    document.getElementById('gnss-mqtt-mini-text').textContent = 'Disconnected';
                    document.getElementById('gnss-mqtt-mini-count').textContent = '0 pos';
                }
                
                // Switch stats panels based on connection status
                const soiStats = document.getElementById('soi-stats-section');
                const gnssStats = document.getElementById('gnss-stats-section');
                if (isConnected && positions.length > 0) {
                    // Show GNSS stats when connected and has data
                    if (soiStats) soiStats.style.display = 'none';
                    if (gnssStats) gnssStats.style.display = 'block';
                } else if (currentView === 'gnss') {
                    // In GNSS view, always show GNSS stats
                    if (soiStats) soiStats.style.display = 'none';
                    if (gnssStats) gnssStats.style.display = 'block';
                } else {
                    // Show SOI stats in map/table view when not connected
                    if (soiStats) soiStats.style.display = 'block';
                    if (gnssStats) gnssStats.style.display = 'none';
                }
                
                if (positions.length === 0) {
                    // Clear existing markers
                    Object.values(gnssReceiverMarkers).forEach(m => m.remove());
                    Object.values(gnssTrackLayers).forEach(l => l.remove());
                    gnssReceiverMarkers = {};
                    gnssTrackLayers = {};
                    return;
                }
                ...
                for (const pos of positions) {
                    const rcvrId = pos.device_id || pos.receiver_id;
                    if (!rcvrId) continue;
                    if (!tracksByReceiver[rcvrId]) {
                        tracksByReceiver[rcvrId] = [];
                    }
                    if (pos.latitude && pos.longitude && Math.abs(pos.latitude) <= 90 && Math.abs(pos.longitude) <= 180) {
                        tracksByReceiver[rcvrId].push([pos.latitude, pos.longitude]);
                        latestByReceiver[rcvrId] = pos;
                    }
                }
                ...
                for (const [rcvrId, track] of Object.entries(tracksByReceiver)) {
                    const color = getReceiverColor(rcvrId);
                    
                    // Update or create track line
                    if (track.length > 1) {
                        if (gnssTrackLayers[rcvrId]) {
                            gnssTrackLayers[rcvrId].setLatLngs(track);
                        } else {
                            gnssTrackLayers[rcvrId] = L.polyline(track, {
                                color: color,
                                weight: 3,
                                opacity: 0.8
                            }).addTo(map);
                    ...
                    if (latest) {
                        ...
                        if (gnssReceiverMarkers[rcvrId]) {
                            gnssReceiverMarkers[rcvrId].setLatLng(latlng);
                        } else {
                            const icon = L.divIcon({
                                className: 'gnss-receiver-marker',
                                html: `<div style="background: ${color}; width: 14px; height: 14px; border-radius: 50%; border: 3px solid white; box-shadow: 0 0 8px rgba(0,0,0,0.5);"></div>`,
                                iconSize: [20, 20],
                                iconAnchor: [10, 10]
                            });
                            
                            gnssReceiverMarkers[rcvrId] = L.marker(latlng, { icon, zIndexOffset: 1000 })
                                .addTo(map)
                                .bindPopup(`<b>📡 ${rcvrId}</b><br>Sats: ${latest.num_sats || '-'}<br>HDOP: ${latest.hdop?.toFixed(2) || '-'}`);
                        }
                    }
                }
                
                // Remove old receivers that no longer exist
                for (const rcvrId of Object.keys(gnssReceiverMarkers)) {
                    if (!tracksByReceiver[rcvrId]) {
                        gnssReceiverMarkers[rcvrId].remove();
                        delete gnssReceiverMarkers[rcvrId];
                        if (gnssTrackLayers[rcvrId]) {
                            gnssTrackLayers[rcvrId].remove();
                            delete gnssTrackLayers[rcvrId];
                        }
                    }
                }
                
                // Handle lock view - pan to locked receiver
                if (gnssLockView && gnssLockedReceiver && latestByReceiver[gnssLockedReceiver]) {
                    const latest = latestByReceiver[gnssLockedReceiver];
                    map.setView([latest.latitude, latest.longitude], gnssLockedZoom, { animate: false });
                }
                
                // Update GNSS stats panel and receiver dropdown
                updateGNSSStatsPanel(positions, latestByReceiver);
                
            } catch (error) {
                console.error('Failed to update GNSS receiver layer:', error);
            }
        }
        
        function updateGNSSStatsPanel(positions, latestByReceiver) {
            const receivers = Object.keys(latestByReceiver);
            document.getElementById('gnss-stat-receivers').textContent = receivers.length;
            document.getElementById('gnss-stat-positions').textContent = positions.length;
            
            // Update receiver dropdown for lock view
            const select = document.getElementById('gnss-lock-receiver');
            if (select) {
                const currentValue = select.value;
                select.innerHTML = '<option value="">Select receiver...</option>';
                for (const rcvrId of receivers) {
                    const opt = document.createElement('option');
                    opt.value = rcvrId;
                    opt.textContent = rcvrId;
                    if (rcvrId === currentValue) opt.selected = true;
                    select.appendChild(opt);
                }
            }
            
            // Calculate avg satellites
            let totalSats = 0, satCount = 0;
            for (const pos of positions) {
                if (pos.num_sats) {
                    totalSats += pos.num_sats;
                    satCount++;
                }
            }
            document.getElementById('gnss-stat-avg-sats').textContent = satCount > 0 ? (totalSats / satCount).toFixed(1) : '-';
            
            // Calculate avg HDOP
            let totalHdop = 0, hdopCount = 0;
            for (const pos of positions) {
                if (pos.hdop) {
                    totalHdop += pos.hdop;
                    hdopCount++;
            ...
            document.getElementById('gnss-stat-hdop').textContent = hdopCount > 0 ? (totalHdop / hdopCount).toFixed(2) : '-';
            
            // Fix rate (positions with fix / total)
            const withFix = positions.filter(p => p.has_fix !== false).length;
            document.getElementById('gnss-stat-fix-rate').textContent = positions.length > 0 ? ((withFix / positions.length) * 100).toFixed(0) + '%' : '-';
            
            // Placeholder for constellation count
            document.getElementById('gnss-stat-constellations').textContent = '-';
        }
        ...
        function loadPOIs() {
            try {
                const stored = localStorage.getItem('ssa_pois');
                if (stored) {
        ...
        function savePOIs() {
            try {
                localStorage.setItem('ssa_pois', JSON.stringify(pois));
            } catch (e) {
        ...
        function addPOI() {
            const label = document.getElementById('poi-label').value.trim() || 'POI ' + (pois.length + 1);
            ...
            const radius1 = parseInt(document.getElementById('poi-radius1').value) || 0;
            const radius2 = parseInt(document.getElementById('poi-radius2').value) || 0;
            const radius3 = parseInt(document.getElementById('poi-radius3').value) || 0;
            ...
            const poi = {
                ...
                lon,
                color,
                radii: [radius1, radius2, radius3].filter(r => r > 0)
            };
            ...
            showNotification(`Added POI: ${label}`, 'success');
        }
        ...
        function clearAllPOIs() {
            if (!confirm('Clear all POIs?')) return;
            pois = [];
            savePOIs();
            renderPOIList();
            updatePOIMapLayers();
            showNotification('All POIs cleared', 'info');
        }
        
        function renderPOIList() {
            const container = document.getElementById('poi-list');
            if (pois.length === 0) {
                container.innerHTML = '<div style="color: var(--text-secondary); font-size: 12px;">No POIs defined. Add one below.</div>';
                return;
            }
            ...
            container.innerHTML = pois.map(poi => `
                <div style="display: flex; align-items: center; gap: 8px; padding: 6px; border-bottom: 1px solid var(--border-color);">
                    <span style="width: 12px; height: 12px; border-radius: 50%; background: ${poi.color};"></span>
                    <span style="flex: 1; font-size: 12px;">${poi.label}</span>
                    <span style="font-size: 10px; color: var(--text-secondary);">${poi.lat.toFixed(5)}, ${poi.lon.toFixed(5)}</span>
                    <button onclick="removePOI(${poi.id})" style="background: none; border: none; color: var(--accent-red); cursor: pointer;">✕</button>
                </div>
            `).join('');
        }
        
        function updatePOIMapLayers() {
            if (!map) return;
            
            // Clear existing
            Object.values(poiMarkers).forEach(m => m.remove());
            Object.values(poiCircles).forEach(circles => circles.forEach(c => c.remove()));
            poiMarkers = {};
            poiCircles = {};
            ...
            for (const poi of pois) {
                ...
                const icon = L.divIcon({
                    ...
                    html: `<div style="background: ${poi.color}; width: 10px; height: 10px; border-radius: 50%; border: 2px solid white; box-shadow: 0 0 4px rgba(0,0,0,0.5);"></div>`,
                    iconSize: [14, 14],
                    iconAnchor: [7, 7]
                });
                
                poiMarkers[poi.id] = L.marker([poi.lat, poi.lon], { icon })
                    .addTo(map)
                    .bindPopup(`<b>${poi.label}</b><br>${poi.lat.toFixed(6)}, ${poi.lon.toFixed(6)}`);
                
                // Radius circles
                poiCircles[poi.id] = [];
                for (const radius of poi.radii) {
                    ...
                    poiCircles[poi.id].push(circle);
                }
            }
        }
        
        // ============= GNSS Receiver Colors =============
        
        function loadGNSSReceiverColors() {
            try {
                const stored = localStorage.getItem('ssa_gnss_colors');
                if (stored) {
                    gnssReceiverColors = JSON.parse(stored);
                    renderGNSSColorList();
                }
            } catch (e) {
                console.error('Failed to load GNSS colors:', e);
            }
        }
        
        function saveGNSSReceiverColors() {
            try {
                localStorage.setItem('ssa_gnss_colors', JSON.stringify(gnssReceiverColors));
            } catch (e) {
                console.error('Failed to save GNSS colors:', e);
            }
        }
        
        function renderGNSSColorList() {
            const container = document.getElementById('gnss-receiver-colors-list');
            const receivers = Object.keys(gnssReceiverColors);
            
            if (receivers.length === 0) {
                container.innerHTML = '<div style="color: var(--text-secondary); font-size: 12px;">No receivers configured. Colors will be auto-assigned.</div>';
                return;
            }
            
            container.innerHTML = receivers.map(rcvr => `
                <div style="display: flex; align-items: center; gap: 8px; padding: 6px; border-bottom: 1px solid var(--border-color);">
                    <input type="color" value="${gnssReceiverColors[rcvr]}" onchange="updateReceiverColor('${rcvr}', this.value)" style="width: 30px; height: 24px; padding: 0; border: none; cursor: pointer;">
                    <span style="flex: 1; font-size: 12px;">${rcvr}</span>
                    <button onclick="removeReceiverColor('${rcvr}')" style="background: none; border: none; color: var(--accent-red); cursor: pointer;">✕</button>
                </div>
            `).join('');
        }
        
        function updateReceiverColor(rcvr, color) {
            gnssReceiverColors[rcvr] = color;
            saveGNSSReceiverColors();
            updateGNSSReceiverLayer();
        }
        
        function removeReceiverColor(rcvr) {
            delete gnssReceiverColors[rcvr];
            saveGNSSReceiverColors();
            renderGNSSColorList();
            updateGNSSReceiverLayer();
        }
        
        function triggerGNSSColorUpload() {
            document.getElementById('gnss-color-upload').click();
        }
        
        async function uploadGNSSColorsJson(file) {
            try {
                const text = await file.text();
                const data = JSON.parse(text);
                gnssReceiverColors = data;
                saveGNSSReceiverColors();
                renderGNSSColorList();
                updateGNSSReceiverLayer();
                showNotification('GNSS colors loaded', 'success');
            } catch (e) {
                showNotification('Failed to load colors file: ' + e.message, 'error');
            }
        }
        
        function exportGNSSColors() {
            const blob = new Blob([JSON.stringify(gnssReceiverColors, null, 2)], { type: 'application/json' });
            const url = URL.createObjectURL(blob);
            const a = document.createElement('a');
            a.href = url;
            a.download = 'gnss_receiver_colors.json';
            a.click();
            URL.revokeObjectURL(url);
        }
        
        // ============= GNSS Export Functions =============
        
        async function exportGNSSToSQLite() {
            try {
                showNotification('Exporting GNSS tracks to SQLite...', 'info');
                
                const response = await fetch('/api/gnss/export/sqlite', { method: 'POST' });
                ...
                if (response.ok) {
                    ...
                    const a = document.createElement('a');
                    a.href = url;
                    a.download = `gnss_tracks_${Date.now()}.sqlite`;
                    a.click();
                    URL.revokeObjectURL(url);
                    showNotification('SQLite export downloaded', 'success');
                } else {
        ...
        async function exportGNSSToGeoJSON() {
            try {
                showNotification('Exporting GNSS tracks to GeoJSON...', 'info');
                
                const response = await fetch('/api/gnss/export/geojson');
                ...
                if (response.ok) {
                    ...
                    const a = document.createElement('a');
                    a.href = url;
                    a.download = `gnss_tracks_${Date.now()}.geojson`;
                    a.click();
                    URL.revokeObjectURL(url);
                    showNotification('GeoJSON export downloaded', 'success');
                } else {
                    // Try to parse as JSON, fall back to text
                    let errorMsg = 'Unknown error';
                    try {
        ...
        // ============= Auto-update GNSS receiver layer =============
        
        let gnssMapUpdateInterval = null;
        
        function startGNSSMapUpdates() {
            if (gnssMapUpdateInterval) return;
            gnssMapUpdateInterval = setInterval(() => {
                if (currentView === 'map') {
                    updateGNSSReceiverLayer();
                }
        ...
        function stopGNSSMapUpdates() {
            if (gnssMapUpdateInterval) {
                clearInterval(gnssMapUpdateInterval);
                gnssMapUpdateInterval = null;
            }
        }
...
def initialize_app(mqtt_config=None, gnss_mqtt_config=None):
    ...
    # Initialize GNSS MQTT if configured
    if gnss_mqtt_config and gnss_mqtt_config.get('host'):
        if initialize_gnss_mqtt(gnss_mqtt_config):
            logger.info(f"GNSS MQTT connected to {gnss_mqtt_config['host']}:{gnss_mqtt_config.get('port', 1883)}")
        else:
            logger.info("GNSS MQTT not available - can connect later via API")
    else:
        logger.info("GNSS MQTT not configured - can connect later via /api/gnss-mqtt/connect")
...
if __name__ == '__main__':
    ...
    # GNSS MQTT config (for live GNSS data from Raspberry Pi)
    gnss_mqtt_config = {
        'host': os.getenv('GNSS_MQTT_HOST', ''),
        'port': int(os.getenv('GNSS_MQTT_PORT', 1883)),
        'username': os.getenv('GNSS_MQTT_USER'),
        'password': os.getenv('GNSS_MQTT_PASS')
    }
    
    initialize_app(mqtt_config, gnss_mqtt_config)
