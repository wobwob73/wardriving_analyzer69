# Wardriving Analyzer v4.4.11

Multi-spectrum RF survey / wardriving analysis tool for Ubuntu 24.

**v4.4.11 notes:** Exposes Route Overview basemap settings in the UI (include map / use basemap tiles / padding) and adds a Settings → About tab that shows the running version/build timestamp.
...
- include Survey Route Overview map image (route extent +10% padding)
- use basemap tiles for the route overview when available

### Report charts
In **Settings → Report Options → Report Charts**:
- Enable/disable charts for **PDF** and/or **HTML**
- Choose which charts to include (signal types, static/mobile, channels)
- Set **Top-N channels** for the channel distribution chart

When enabled, reports include three small **pie charts** near the top to summarize the selected dataset.
